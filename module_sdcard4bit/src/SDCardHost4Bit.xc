...
#ifdef _STREAM_FS
/**********************************************************************************
 * STREAMED version of commands
 **********************************************************************************/

static DRESULT SendCmd_streamed(BYTE IfNum, BYTE Cmd, DWORD Arg, RESP_TYPE RespType, int DataBlocks, streaming chanend c, RESP Resp)
{ //01CMD[6]ARG[32]CRC[7]1
  unsigned int i, j, Crc0 = 0, Crc1, Crc2, Crc3;
  unsigned int D0, D1, D2, D3;
  unsigned int RespStat, RespBitLen, RespBitCount, RespByteCount;
...
DRESULT disk_write_streamed(BYTE IfNum, streaming chanend c, DWORD sector, BYTE count)
{
  ...
  { // multiblock write
    //if(SendCmd(SDif, 23, NumBlocks, R1, 0, DummyData, Resp)) return 0; // set foreseen multiple block read. Remarked because only optionally supported by cards
    if(SendCmd_streamed(IfNum, 25, SDif[IfNum].Ccs ? sector : 512 * sector, R1, -count, c, Resp)) return RES_ERROR; // multiblock write
    if(SendCmd(IfNum, 12, 0, R1B, 0, DummyData, Resp)) return RES_ERROR; // stop multi-block write. (using stop command instead of cmd23)
  }
  else
    if(SendCmd_streamed(IfNum, 24, SDif[IfNum].Ccs ? sector : 512 * sector, R1, -1, c, Resp)) return RES_ERROR; // single block write
  return RES_OK;
}
#endif
